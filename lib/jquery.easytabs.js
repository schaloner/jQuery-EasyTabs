  ...
  $.easytabs = function(container, options) {
    ...
    plugin.selectTab = function($clicked, callback) {
      var url = window.location,
        hash = plugin.trimHash(url.hash.match(/^[^\?]*/)[0]),
    ...
    // Remove any path parameters that form part of the hash - these cause issues with Backbone
    plugin.trimHash = function(hash) {
    ...
    plugin.matchTab = function(hash) {
      return plugin.tabs.find("[href='" + plugin.trimHash(hash) + "'],[data-target='" + hash + "']").first();
    };

    // Find panel with `id` matching value
    plugin.matchInPanel = function(hash) {
      return ( hash ? plugin.panels.filter(':has(' + plugin.trimHash(hash) + ')').first() : [] );
    };

    // Select matching tab when URL hash changes
    plugin.selectTabFromHashChange = function() {
      var hash = plugin.trimHash(window.location.hash.match(/^[^\?]*/)[0]),
    ...
    var activateTab = function($clicked, $targetPanel, ajaxUrl, callback) {
      ...
      if( fire($container,"easytabs:before", [$clicked, $targetPanel, settings]) ){
        var $visiblePanel = plugin.panels.filter(":visible"),
          ...
          heightDifference,
          showPanel,
          hash = plugin.trimHash(window.location.hash.match(/^[^\?]*/)[0]);
